...
export const getTrajet = async (idDossier, idTrajet) => {
    try {
        const response = await fetch(`http://192.168.1.22/api/reservation/${idDossier}/${idTrajet}`);
...
export const changeTrajetStatue = async (idDossier, idTrajet, status) => {
    ...
    try {
        ...
        if (status === 0) {
            endpoint = `http://192.168.1.22/api/reservation/setOngoing/${idDossier}/${idTrajet}`;
        } else if (status === 1) {
            endpoint = `http://192.168.1.22/api/reservation/setDone/${idDossier}/${idTrajet}`;
        } else {
